// js/equipamentos.js - LIMPO (controle no utils.js)
...
function renderizarCards(equipamentos) {
    ...
    equipamentos.forEach(eq => {
        ...
        html += `<div class="bg-zinc-800 p-2 rounded-lg border border-zinc-700"><div class="flex justify-between"><a href="http://${eq.ip}" target="_blank" class="text-emerald-400 hover:underline font-bold text-xs truncate">${eq.nome || '-'}</a><span class="${cls} text-xs font-bold ml-1">${icone}</span></div><div class="text-zinc-500 text-xs">🌐 ${eq.ip || '-'}:${eq.porta || '80'}</div><div class="text-zinc-500 text-xs">📍 ${eq.localidade || '-'}</div><div>${modo}</div>${p2pInfo}<div>👥 ${eq.clientes || 0} | ⏱️ ${latencia}</div><div class="flex justify-end gap-1 mt-1"><button onclick="editarEquipamento(${eq.id})" class="text-blue-400 text-xs">✏️</button><button onclick="excluirEquipamento(${eq.id}, '${(eq.nome || '').replace(/'/g, "\\'")}')" class="text-red-400 text-xs">🗑️</button></div></div>`;
    });
    html += '</div>';
...
// ========== TOGGLE P2P / SSH ==========
function toggleP2P() { document.getElementById('p2pFields').classList.toggle('hidden', document.getElementById('formModo').value !== 'p2p'); if (document.getElementById('formModo').value === 'p2p') carregarParesP2P(); }
function carregarParesP2P() {
    let p2pTipo = document.querySelector('input[name="p2pTipo"]:checked');
    if (!p2pTipo) { document.querySelector('input[name="p2pTipo"][value="ap"]').checked = true; p2pTipo = document.querySelector('input[name="p2pTipo"][value="ap"]'); }
...
function fecharModal() { document.getElementById('modalEquipamento').classList.add('hidden'); }
...
async function editarEquipamento(id) {
    ...
    if (eq.p2p_tipo) { let radio = document.querySelector(`input[name="p2pTipo"][value="${eq.p2p_tipo}"]`); if (radio) radio.checked = true; }
...
async function salvarEquipamento() {
    ...
    let p2pTipo = '', p2pPar = '';
    if (modo === 'p2p') { let radio = document.querySelector('input[name="p2pTipo"]:checked'); p2pTipo = radio ? radio.value : 'ap'; p2pPar = document.getElementById('formP2PPar').value; }
    let dados = { nome: document.getElementById('formNome').value, ip: document.getElementById('formIp').value, localidade: document.getElementById('formLocalidade').value, modo_operacao: modo, tipo: 'equipamento', porta: document.getElementById('formPorta').value || '80', p2p_tipo: p2pTipo, p2p_par: p2pPar, ssh_enabled: document.getElementById('formSsh').checked, ssh_usuario: document.getElementById('formSshUser').value || 'ubnt', ssh_senha: document.getElementById('formSshPass').value, ssh_porta: parseInt(document.getElementById('formSshPort').value) || 22, dados_snmp: {} };
